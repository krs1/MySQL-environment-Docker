...
-- 50
SELECT COUNT(*) AS 件数 FROM 口座 WHERE 種別 != '1' AND 残高 >= 1000000
AND 更新日 < '2024-01-01'
...
FROM 口座 GROUP BY SUBSTRING( 口座番号 , 7, 1) ORDER BY 件数 DESC

-- 56
SELECT SUBSTRING(COALESCE(更新日, 'XXXX'), 1, 4) AS 更新年, SUM(残高) AS 合計, MAX(残高) AS 最大, MIN(残高) AS 最小, AVG(残高) AS 平均 , COUNT(*) AS 件数
FROM 口座
GROUP BY SUBSTRING(COALESCE(更新日, 'XXXX'), 1, 4)

-- 57
SELECT 種別 , SUM(残高) AS 合計 , COUNT(*) AS 件数 FROM 口座 GROUP BY 種別
HAVING SUM( 残高 ) > 3000000

-- 58
SELECT SUBSTRING(名義, 1, 1) AS 名義, COUNT(名義) AS 件数, AVG(CHAR_LENGTH(REPLACE( 名義 , '　', ''))) AS 文字数の平均
FROM 口座 GROUP BY SUBSTRING( 名義 , 1, 1) 
