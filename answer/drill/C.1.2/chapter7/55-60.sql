...
FROM 注文
WHERE 商品コード = 'S0604') AS 数量 FROM 商品 WHERE 商品コード = 'S0604'
...
-- 57
SELECT 注文日 , 商品コード FROM 注文 WHERE 商品コード IN (
    SELECT 商品コード FROM 商品
    WHERE 商品名 LIKE '%あったか%')
ORDER BY 注文日
...
-- 59
SELECT A.数量合計 AS 割引による販売数, TRUNCATE(A.割引料合計 / A.数量合計 , 0) AS 平均割引額
FROM (
    SELECT SUM(数量) AS 数量合計, SUM(クーポン割引料) AS 割引料合計
    FROM 注文
...
FROM 注文
WHERE 注文日 = '2024-03-22' AND 注文番号 = '202403220901'
GROUP BY 注文日 , 注文番号